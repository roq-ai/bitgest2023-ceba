...
import { getTableStructureById, updateTableStructureById } from 'apiSdk/table-structures';
...
function TableStructureEditPage() {
  const router = useRouter();
  const id = router.query.id as string;

  const { data, error, isLoading, mutate } = useSWR<TableStructureInterface>(
    () => (id ? `/table-structures/${id}` : null),
    () => getTableStructureById(id),
  );
  ...
  const handleSubmit = async (values: TableStructureInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateTableStructureById(id, values);
      mutate(updated);
      resetForm();
