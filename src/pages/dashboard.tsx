...
function DashboardPage() {
  ...
  const formStat = useSWR<PaginatedInterface<FormInterface>>(
    () => `/forms`,
    () => getForms({ limit: 1, offset: 0 }),
  );

  const tableStructureStat = useSWR<PaginatedInterface<TableStructureInterface>>(
    () => `/table-structures`,
    () => getTableStructures({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Forms',
      result: formStat?.data?.totalCount,
      isLoading: formStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Table Structures',
      result: tableStructureStat?.data?.totalCount,
      isLoading: tableStructureStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
