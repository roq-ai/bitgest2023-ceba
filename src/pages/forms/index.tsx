...
export function FormListPage(props: FormListPageProps) {
  ...
  const fetcher = useCallback(
    async () =>
      getForms({
        relations: ['table_structure', 'company'],
        limit: params.pageSize,
        offset: params.pageNumber * params.pageSize,
        searchTerm: params.searchTerm,
        order: params.order,
        searchTermKeys: ['form_type.contains'],
        ...(params.filters || {}),
      }),
  ...
  const columns: ColumnType[] = [
    { id: 'form_type', header: 'Form Type', accessorKey: 'form_type' },
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Link
              ...
              href={`/table-structures/view/${record.table_structure?.id}`}
            >
              {record.table_structure?.table_name}
            </Link>
          ),
        }
      : null,
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/forms/view/${record.id}`} passHref legacyBehavior>
                <Button
                  onClick={(e) => e.stopPropagation()}
                  mr={2}
                  padding="0rem 8px"
